...
> **专为 LLM Skills 编写而生的简洁、精炼、无歧义 结构化提示词。**

## 1. 简介

**promptica** 采用领域专用语言（DSL）的思路，专为编写结构化、可靠且可扩展的 [Claude Skills](https://docs.claude.com) 而设计 —— 指引 LLM 执行特定工作流程、领域知识与可重复任务的指令集。

当 LLM Skills 出现时，我看到了一个熟悉的模式：一项强大的能力，因缺乏合适的语言来表达而受到束缚。自由格式的 Markdown 是起点，而非终点。**promptica** 是我的一个尝试。
...
经历过足够多的技术周期之后，你会开始认识到某个时刻——一项新能力开始超越它原本的脚手架。LLM Skills 正处于这个时刻。
...
`->` 箭头刻意省略 —— `{ }` 块分隔符本身已足以标示每个 case 主体的起始位置，在不牺牲清晰度的前提下保持语法精炼。此构件自然对应真实世界的 LLM Skill 路由场景，即 Skill 需要识别用户意图并分发至对应处理逻辑的情境。
...
LLM Skill 工作流程时常遭遇难以预测的情况 —— 超时、模糊输入、缺少上下文。将这些情况视为可捕获、可处理的异常，而非无声的失败，能使 Skills 在生产环境中显著更加健壮。
...
- 提升 LLM Skills 的**可读性、可维护性与可靠性**
- 从简单的单一任务 Skill 扩展至复杂的多步骤工作流程，无需全面重写

最终带来更快的迭代速度、更清晰的意图表达，以及更健壮的 LLM 驱动体验。
